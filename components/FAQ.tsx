...
export default function FAQ() {
  ...
  const growthLabDefinition = getDefinitionSchema({
    name: "GrowthLab",
    description:
      "A global startup ecosystem that empowers founders, investors, students, and innovators to connect, launch, and grow.",
    definition:
      'GrowthLab is a global startup ecosystem platform that combines community, education, funding, and technology to help early-stage entrepreneurs turn ideas into scalable ventures. It functions like "LinkedIn for startups" with networking, business pages, job posting, and fundraising capabilities.',
    alternateNames: [
      "GrowthLab Singapore",
      "GrowthLab Platform",
      "GrowthLab Startup Community",
    ],
  });

  // Generate SpeakableSpecification for voice search
  const speakableSchema = getSpeakableSpecification({
    cssSelector: ['h2:contains("FAQs")', ".faq-question", ".faq-answer"],
  });

  return (
    <section
      id="faq"
      className="framer-motion-optimized relative py-16 sm:py-24 lg:py-32 xl:py-40 bg-white overflow-hidden"
    >
      <StructuredData
        data={[faqSchema, qaPageSchema, growthLabDefinition, speakableSchema]}
      />
          ...
          {faqs.map((faq, index) => (
            ...
            >
              <button
                ...
                className="w-full py-6 px-6 text-left flex items-center justify-between group min-h-[44px] transition-colors duration-300"
              >
                <span
          ...
          <div className="bg-gradient-to-br from-primary/10 to-amber/10 rounded-2xl p-10 border border-primary/20">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
              <a
                ...
                className="inline-flex items-center gap-2 px-8 py-4 bg-primary text-white font-semibold rounded-lg transition-all duration-300 min-h-[48px] shadow-lg"
              >
                <Mail size={18} />
                hello@growthlab.sg
              </a>
              <a
                href="tel:+6597371722"
                className="inline-flex items-center gap-2 px-8 py-4 bg-white text-slate-900 font-semibold border-2 border-slate-300 rounded-lg transition-all duration-300 min-h-[48px]"
              >
                +65 9737 1722
              </a>
            </div>
